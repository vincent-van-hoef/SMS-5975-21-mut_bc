...
- Version 1: Most results included, but sections on nanostring correlation and ESR1 mutation remain to be added.
...
The tumor mutational burden metric gives a general and rough indication of the amount of mutation in a specific tumor. It is defined as the number of mutations per Mb. The filtering strategy will influence this metric quite strongly but ther eis no consensus on a specific strategy. Here synonymous mutations have been kept, while cancer speific mutations have been filtered out, as is described in the software manual.
...
TMB does not seem to differ significantly between the different tumor locations, see @fig-tmb-loc.
...
```{r}
pt_surv <- box_read_csv("955734268821") 
...
```{r}
dt_surv <- box_read_csv("955747088113") 
...
```{r}
ln_surv <- box_read_csv("955741332486") 
