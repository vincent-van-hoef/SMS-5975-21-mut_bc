...
An attempt was made to link the mutation data can be linked to the survival data. For this, samples are divided in two groups: samples with a mutation for a certain gene and another group of samples that are WT for the gene. The endpoint is Overall Survival.
...
```{r}
pt_surv <- box_read_csv("956229079317") 
datatable(pt_surv, rownames = FALSE, extensions = c('Buttons', 'Scroller'), options = list(dom = 'Bfrtip', buttons = c("copy", "csv", "excel"), deferRender = TRUE, scrollY = 200, scroller = TRUE, order = list(list(5, 'desc'))))
```

And as KM-curve below
...
These are the results for the survival analysis within distant tumors only.

```{r}
dt_surv <- box_read_csv("956229079317") 
datatable(dt_surv, rownames = FALSE, extensions = c('Buttons', 'Scroller'), options = list(dom = 'Bfrtip', buttons = c("copy", "csv", "excel"), deferRender = TRUE, scrollY = 200, scroller = TRUE, order = list(list(5, 'desc'))))
```

And as KM-curve below

```{r}
fl <- list.files(path = "", pattern = "_survival_curve_PT_3_common.svg")
fl <- as.list(gsub("_survival_curve_PT_3_common.svg", "", fl))
...
html`<img src="./Results/variant_calls_all/merged_calls/Plots/3_common/survival_analysis/${genes_surv_dt}_survival_curve_PT_3_common.svg" width:100% height:600px></img>`
```
...
```{r}
ln_surv <- box_read_csv("956229079317") 
datatable(ln_surv, rownames = FALSE, extensions = c('Buttons', 'Scroller'), options = list(dom = 'Bfrtip', buttons = c("copy", "csv", "excel"), deferRender = TRUE, scrollY = 200, scroller = TRUE, order = list(list(5, 'desc'))))
```
...
html`<img src="./Results/variant_calls_all/merged_calls/Plots/3_common/survival_analysis/${genes_surv_ln}_survival_curve_PT_3_common.svg" width:100% height:600px></img>`
```
