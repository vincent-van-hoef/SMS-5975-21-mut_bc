...
```{ojs}
all_genes = FileAttachment("./Results/CNA/distant_tumor/all_genes.csv").csv({typed: true})
columns = [0,1,2,3,4,15].map(x=>all_genes.columns[x])
```

```{ojs}
//| panel: input
viewof gene = Inputs.select(all_genes.map(d => d.genesymbol), {unique: true, label: "Select a gene: "})
viewof type = Inputs.checkbox(["AMPLIFICATION", "DELETION", "NA"], {label: "Select type of alteration: ", value: ["AMPLIFICATION", "DELETION"]})
```

```{ojs}
filtered = all_genes.filter(function(dat) { return gene.includes(dat.genesymbol) && type.includes(dat.type)})
```
...
<a href="./Results/distant_tumor_copy_number_data.tar.gz" download><button class="btn"><i class="fa fa-download"></i> Download Distant Metastases CNA data</button></a>

::: {#fig-cna-dm layout-ncol=2 height="1500px"}

![Amplified](./Results/CNA/distant_tumor/Amplified_in_distant_tumor_vs_primary_tumor.png){#fig-cna-dm-amp}

![Deleted](./Results/CNA/distant_tumor/Deleted_in_distant_tumor_vs_primary_tumor.png){#fig-cna-dm-del}
...
![Amplified](./Results/CNA/lymph_node/Amplified_in_lymph_node_vs_primary_tumor.png){#fig-cna-lnm-amp}

![Deleted](./Results/CNA/lymph_node/Deleted_in_lymph_node_vs_primary_tumor.png){#fig-cna-lnm-del}
